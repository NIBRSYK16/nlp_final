...
### 1. 本地代码生成
- 支持 Qwen2.5-Coder-0.5B/0.5B 本地模型部署
- 提供交互式代码生成界面
...
5. **模型微调**：使用收集的高质量数据微调本地 0.5B 模型

#### 特点
- **自动化数据收集**：无需人工标注，自动筛选高质量代码样本
- **多模型协作**：32B 生成 + 14B 验证 + 0.5B 部署的混合架构
- **持续优化**：通过微调实现模型的持续改进
...
#### 核心功能
1. **指令合成（Instruction Synthesis）**
   - 从代码片段自动生成编程问题指令
   - 支持直接合成和优化驱动合成两种模式
   - 使用通义千问 API 进行高质量指令生成

2. **复杂度评分（Complexity Scoring）**
   - 自动评估生成指令的复杂度
   - 使用 LLM 对指令进行复杂度打分（0-10分）
   - 筛选高质量、有挑战性的编程问题

3. **多样性评分（Diversity Scoring）**
   - 评估指令之间的多样性
   - 使用嵌入模型计算语义相似度
   - 确保生成的数据集具有足够的多样性

4. **多轮演化流程**
   - **第1轮**：从种子代码片段生成初始指令（直接合成）
   - **第2轮**：基于复杂度和多样性评分进行优化驱动演化
   - 自动过滤低质量数据（复杂度>=6.5，多样性>=0.6）
...
#### 特点
- **自动化生成**：从代码片段自动生成编程问题，无需人工编写
- **质量保证**：通过复杂度和多样性评分确保生成指令的质量
- **成本优化**：使用通义千问 API，成本相对较低
- **快速迭代**：简化版设计，快速生成约500条高质量数据
...
#### 高质量指令生成（simplified_toe_qwen）
1. **准备种子数据**
   - 创建 `simplified_toe_qwen/data/seed/seed_samples.json`
   - 格式：包含代码片段的 JSON 数组

2. **运行完整流程**
   ```bash
   cd simplified_toe_qwen
   python run_pipeline.py --seed_samples 50 --target_samples 500
   ```

3. **生成结果**
   - 最终数据保存在 `data/final/final_dataset_*.json`
   - 包含复杂度评分和多样性评分
   - 可用于自我演化系统的训练数据
...
- **多模型协作**：利用不同规模模型的优势（32B生成、14B验证、0.5B部署）
...
### 5. 高质量指令生成
- **Tree-of-Evolution方法**：基于演化思想生成多样化、高质量的编程指令
- **自动化流程**：从代码片段到指令的完整自动化生成
- **质量保证**：通过复杂度和多样性评分确保生成质量
- **与自我演化集成**：生成的指令可直接用于自我演化系统的训练

## 项目结构

```
nlp_final/
├── gradio_local_model.py      # 主程序文件（旧版）
├── optimized.py               # 优化版主程序（包含批量自我演化）
├── main.py                    # 重构版主程序入口
├── config.py                  # 配置管理
├── models/                    # 模型文件目录
...
│   ├── fine_tune_manager.py   # 微调管理模块
│   └── evolution_core.py     # 自我演化核心逻辑
├── simplified_toe_qwen/       # 高质量指令生成模块
│   ├── run_pipeline.py        # 完整流程脚本
│   ├── src/
│   │   ├── instruction_synthesis.py  # 指令合成
│   │   ├── complexity_scoring.py     # 复杂度评分
│   │   ├── diversity_scoring.py      # 多样性评分
│   │   ├── llms/
│   │   │   └── qwen_client.py       # 通义千问客户端
│   │   └── prompts/                  # 提示词模板
│   └── README.md              # 模块说明文档
├── evolution_training_data/   # 自我演化训练数据
...
- **评估基准**：HumanEval
- **指令生成**：Tree-of-Evolution 方法（simplified_toe_qwen）
- **API服务**：通义千问 DashScope API
